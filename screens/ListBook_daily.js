...
export default function ListBook_daily({route,navigation}) {

    const [house, setHouse] = useState('');

    const load  = async() =>{
        let uid = await AsyncStorage.getItem("uid");
        fetch('https://app.rthouse.ml/listbook_daily.php?uid='+uid)
...
<FlatList 
              ...
              renderItem={({key,item}) =>
<ListItem key={key}  bottomDivider topDivider onPress={() => navigation.navigate('Detail_tem',{id:item.id, idh:item.idh, name: item.name})}>
      <FontAwesome5 name="home" size={20} color="#333" />
        ...
        <ListItem.Chevron />
              ...
              : <Text style={{fontFamily:'Kanits',fontSize:18,textAlign: 'center',color:'#000',paddingTop:7}}>ไม่มีข้อมูลการเช่า</Text>}
</ScrollView>
