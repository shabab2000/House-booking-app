...
export default function Reserve({route,navigation}) {
    ...
    const load  = async() =>{
        ...
        fetch('https://app.rthouse.ml/reserve.php?uid='+uid)
  ...
  <Appbar.Content title="รายการชำระเงินมัดจำ" color="white" />
...
<FlatList 
              ...
              renderItem={({key,item}) =>
<ListItem key={key}  bottomDivider topDivider onPress={() => navigation.navigate('Detail_reserve',{name:item.name, deposit:item.deposit, img:item.img, uname:item.uname, paydate:item.paydate,status:item.status, pid:item.id,idb:item.idb,idh:item.idh,nameb:item.nameb,numbers:item.numbers,bank:item.bank})}>
