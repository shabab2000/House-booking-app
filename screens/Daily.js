...
export default function Daily({navigation}) {
 ...
 const load  =() =>{
    fetch('https://app.rthouse.ml/daily.php')
  ...
  <Appbar.Content title="ค้างคืน" color="white" />
</Appbar.Header>
            ...
            <FlatList 
              ...
              renderItem={({key,item}) =>
        <ListItem key={key}  bottomDivider topDivider onPress={() => navigation.navigate('Daily_details',{id: item.id, name: item.name, price: item.price, address: item.address, status: item.status,  details: item.details, tel: item.tel, id_owner: item.id_owner, id_img: item.id_img})}>
            ...
            <ListItem.Title><Text style={{fontSize:14,fontFamily:'Kanits',paddingLeft:10}}>ให้เช่า {item.price}/คืน</Text></ListItem.Title>
