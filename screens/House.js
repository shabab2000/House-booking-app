...
export default function House({navigation}) {
 ...
 const load  =() =>{
    fetch('https://app.rthouse.ml/house.php')
  ...
  <Appbar.Content title="บ้านเช่า" color="white" />
</Appbar.Header>
            ...
            <FlatList 
              ...
              renderItem={({key,item}) =>
        <ListItem key={key}  bottomDivider topDivider onPress={() => navigation.navigate('Detail_house',{id: item.id, name: item.name, price: item.price, address: item.address, bathroom: item.bathroom, bedroom: item.bedroom,status: item.status, rule: item.rule, facility: item.facility, details: item.details, tel: item.tel, id_owner: item.id_owner, id_img: item.id_img,deposit: item.deposit})}>
