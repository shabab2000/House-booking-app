...
export default function Detail_tem({route,navigation}) {
    ...
    const load  = async() =>{
        let uid = await AsyncStorage.getItem("uid");
        fetch('https://app.rthouse.ml/detail_tem.php?uid='+uid+'&id='+route.params.id)
            .then((response) => response.json())
...
<View style={{paddingTop:20,paddingBottom:20}}>
    ...
    <View style={{display: 'flex',flexDirection: 'row',}}>
        ...
        :<View style={{paddingTop:20,justifyContent: 'center',alignItems: 'center'}}>
               ...
               onPress={()=> house.category == 'ชั่วคราว'? navigation.replace('Payment_tem',{name: route.params.name, price: house.price, deposit: house.deposit, id: house.id, id_bk: ids}):navigation.replace('Payment_daily',{name: route.params.name, price: house.price, deposit: house.deposit, id: house.id, id_bk: ids})} >
                   <Text style={{fontSize:18,fontFamily:'Kanits',color:'white',paddingHorizontal: 20}}>ชำระเงิน</Text>
               </TouchableOpacity>
           </View>
        }
    <Text style={{fontSize:25,fontFamily:'Kanits',textAlign: 'center',paddingTop:20}}>ข้อมูลห้องพัก</Text>
    <View style={{paddingHorizontal:20,paddingTop:15}}>

        <Text style={{fontSize:18,fontFamily:'Kanits'}}>{route.params.name}</Text>
        <Text style={{fontSize:18,fontFamily:'Kanits'}}>ค่าเช่า {house.price}฿/{house.category =='ชั่วคราว'? 'ชั่วโมง':'คืน'}</Text>
        <Text style={{fontSize:18,fontFamily:'Kanits'}}>ที่อยู่ {house.address}</Text>
        <Text></Text>
        <Text style={{fontSize:18,fontFamily:'Kanits',paddingLeft:10}}>{house.details}</Text>
